...
import { payrollDeductionsBreakdownQuerySchema } from "@/schemas/reports";
...
export async function GET(req: NextRequest) {
  return withRouteErrorHandling(async () => {
    ...
    const parsed = payrollDeductionsBreakdownQuerySchema.safeParse(
    ...
    if (q.format === "csv") {
      const csv = [
        ["Deduction Type", "Total Amount", "Count"].join(","),
        ...data.map((r) => [r.type, r.totalAmount, r.count].join(",")),
      ].join("\n");

      return new Response(csv, {
        headers: {
          "Content-Type": "text/csv",
          "Content-Disposition": `attachment; filename="deductions-breakdown-${new Date().toISOString().split("T")[0]}.csv"`,
        },
      });
    }
    ...
    if (q.format === "pdf") {
      ...
      const pdfBuffer = await generateReportPdf({
        title: "Deductions Breakdown Report",
        subtitle: "Breakdown of deductions by type",
      ...
      return new Response(new Uint8Array(pdfBuffer), {
        headers: {
          ...
          "Content-Disposition": `attachment; filename="deductions-breakdown-${new Date().toISOString().split("T")[0]}.pdf"`,
        },
      });
    }
